<%@page import="model.affectation_fn"%>
...
<%  if (request.getParameter("command") != null) {
        ...
        if (comm.equals("LOAD")) {
            aff = (affectation_fn) request.getAttribute("THE_AFFECTATION");
        } else {
            comm = "";
        }
    } else {
                    ...
                    <ul class="navbar-nav mr-auto">
                        ...
                        <li class="nav-item dropdown nav-item selected">
                            ...
                            <div class="dropdown-menu" aria-labelledby="navbarDropdown4">
                                ...
                                <div class="dropdown-divider"></div>
                                <a class="dropdown-item active" href="AffectationServlet_fn">AFFECTATION DES FOURNITURES</a>
                ...
                <div class="container">
                    ...
                    <div class="clearfix">
                        ...
                        <div class="float-left">
                            ...
                            <form  class="form-inline my-2 my-lg-0" action="AffectationServlet_fn" >
                                <%if (comm.equals("LOAD")) {%>
                                <input type="hidden" name="command"  value="ADD">
                                <input type="hidden" name="idmem"  value="<%=aff.getIdmembre()%>">
                                <input type="hidden" name="idfn"  value="<%=aff.getIdfourniture()%>">
                                <%} else {%>
                                <input type="hidden" name="command"  value="ADD">
                                <%}%>
                                ...
                                <select class="custom-select form-control my-2 mr-sm-2 " name="fourniture">
                                    <%if (!comm.equals("LOAD")) {%> <option selected>Choisir une Fourniture</option> <%}%>
                                    <c:forEach var="m" items="${fournitures}">
                                        <%if (comm.equals("LOAD")) {%>      <c:if test="${m.id!=THE_AFFECTATION.idfourniture}">   <option value="${m.id}">${m.designation}</option></c:if>
                                        <c:if test="${m.id==THE_AFFECTATION.idfourniture}">   <option selected value="${m.id}">${m.designation}</option>></c:if><%} else {%>
                        ...
                        <tbody> 
                            <c:forEach var="temp" items="${AFFECTATION_LIST}">
                                ...
                                <c:url var="tempLink" value="AffectationServlet">
                                    ...
                                    <c:param name="fournitureid" value="${temp.getIdfourniture()}"/>
                                </c:url>
                                ...
                                <tr>
                                    ...
                                    <td>    
                                        <a  href="AffectationServlet_fn?command=LOAD&membreid=${temp.getIdmembre()}&fournitureid=${temp.getIdfourniture()}" ><i class="ion-settings"></i></a>

                                        <a class="del"  href="AffectationServlet_fn?command=DELETE&membreid=${temp.getIdmembre()}&fournitureid=${temp.getIdfourniture()}"><i class="ion-android-delete"></i></a>	
                                    </td>
                                </tr>
